...
- 今日任务页支持快速双击确定键，全屏查看当前选中的任务；全屏页仍可用上下键切换任务，
  再次双击确定键返回，不改变原有单击确定键进入设置的操作。
- 去掉 Passport 任务看板中的数字百分比，目标进度仍通过进度条展示。
...
- Passport 回复语音默认使用满音量；对话页将带用户代号的发言放在右侧、总部回复放在
  ...
  iPhone。
- 持久保存加密联动特征使用的 iPhone BLE 配对密钥，避免普通 App 重装或重连后
  CoreBluetooth 与 Passport 的密钥状态不一致、必须反复“忽略此设备”。
